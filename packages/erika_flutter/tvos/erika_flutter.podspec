...
case "${PLATFORM_NAME:-appletvos}" in
  appletvos)
    RUST_TARGETS="aarch64-apple-tvos"
...
if [ -n "${ERIKA_TVOS_CAPI_STATICLIB:-}" ]; then
  ...
  for RUST_TARGET in $RUST_TARGETS; do
    case "$RUST_TARGET" in
      aarch64-apple-tvos) BINDGEN_CLANG_TARGET="arm64-apple-tvos" ;;
      aarch64-apple-tvos-sim) BINDGEN_CLANG_TARGET="arm64-apple-tvos-simulator" ;;
      x86_64-apple-tvos) BINDGEN_CLANG_TARGET="x86_64-apple-tvos-simulator" ;;
    esac
...
case " $OUTPUT_ARCHS " in
  *" arm64 "*) ;;
  *) echo "error: Erika tvOS runtime is missing arm64: $OUTPUT_ARCHS" >&2; exit 1 ;;
esac
if [ "${PLATFORM_NAME:-appletvos}" = "appletvsimulator" ]; then
  case " $OUTPUT_ARCHS " in
    *" x86_64 "*) ;;
    *) echo "error: Erika tvOS simulator runtime is missing x86_64: $OUTPUT_ARCHS" >&2; exit 1 ;;
  esac
fi
